...
population = []
fitnessPopulation =[]
imageOrigFileName = os.path.splitext(origImagePath)[0]
#program
# Get original image
...
class New_Toplevel:
    ...
    def createRandomImage(self,imgOrig):
        ...
        for i in range(0, rows):
            y =0
            for j in range(0,cols):
                randIndx = randint(0,len(imageLib)-1)
                chromosome[i,j]= imageLib[randIndx]        
        return chromosome
    ...
    def mutation (self,p1):
        ...
        for i in range(0,mutationAmt):
            ...
            randIndx = randint(0,len(imageLib)-1)
            offspring[xIndx][yIndx]= imageLib[randIndx]
        return offspring
    ...
    def updateGUI(self):
        if (genNumber==0):
            image = cv2.imread(os.path.join(outputFolder,'gen0.jpg'))
        else:
            image = cv2.imread(os.path.join(outputFolder,'gen'+str(genNumber-1)+'.jpg'))
        
        if image is not None:
            if (len(fitnessPopulation)>0 ):
                self.fitnesslbl["text"]='Fitness: '+str(round(fitnessPopulation[0],1))
            
            if (str(genNumber)!= self.genNumber_11["text"]):
            ...
            self.genImage.image= genImg

    def run(self,queueArgs):
        global population,fitnessPopulation,genNumber
        self.getUserInput()
        print("Done Setup....\n\n")
        ...
        while True:
            ...
            fitnessPopulation = self.fitness(imageOrig,population,segImage)
            #Allow main Gui thread and GA thread to communicate and quite on button press
            if not queueArgs.empty():
                h = queueArgs.get()
                if (h=="quit"):
                    self.saveFileToOuputFolder()
                    print("Genetic Algorithm Thread shutting down...")
                    queueArgs.task_done()
                    break
                else:
                    queueArgs.task_done()
    ...
    #Enable multithreaded workload to stop gui "freezing"
    def startThread(self):
        self.ButtonStart.configure(state="disabled")
        self.ButtonStop.configure(state="normal")
        global submit_thread,queue
        queue = Queue()
        print("Starting Genetic Algorithm thread...")
        submit_thread = threading.Thread(target=self.run,args=(queue,))
        submit_thread.daemon = True
        submit_thread.start()
        root.after(500, self.checkThread)
    ...
    def getUserInput(self):
        global pop_size, tournament_size, mutationProb, mutationAmount, crossOverProb
        pop_size = int(self.popSize.get())
        tournament_size = int(self.tourSize.get())
        mutationAmount = int(self.mutationSize.get())
        crossOverProb = int(self.varCrossProb.get())/100.0
        mutationProb = int(self.varMutProb.get())/100.0
        print("pop: "+str(pop_size)+" tour "+str(tournament_size)+" mutA"+str(mutationAmount)+" cross"+str(crossOverProb)+" mutP"+str(mutationProb))
        
    
    def saveFileToOuputFolder(self):
        global outputFolder
        image = cv2.imread(os.path.join(outputFolder,'gen'+str(genNumber)+'.jpg'))
        cv2.imwrite(os.path.join(output_dir,imageOrigFileName+'_mosaic.jpg'),image)
    ...
    def __init__(self, top=None):
        ...
        self.fitnesslbl.place(relx=0.88, rely=0.08, height=19, width=180)
        ...
        self.crossoverProbI = Scale(self.Labelframe1_7,variable=self.varCrossProb)
        self.crossoverProbI.place(relx=0.56, rely=0.11, relwidth=0.42
                , relheight=0.0, height=65)
        self.crossoverProbI.configure(activebackground="#d9d9d9")
        self.crossoverProbI.configure(font=font11)
        self.crossoverProbI.configure(label="Cross-over Probability (%)")
        self.crossoverProbI.configure(length="442")
        self.crossoverProbI.configure(orient="horizontal")
        self.crossoverProbI.configure(troughcolor="#d9d9d9")
        self.crossoverProbI.set(round((crossOverProb)*100))
        ...
        self.mutationProbI = Scale(self.Labelframe1_7,variable=self.varMutProb)
        self.mutationProbI.place(relx=0.56, rely=0.45, relwidth=0.42
                , relheight=0.0, height=65)
        self.mutationProbI.configure(activebackground="#d9d9d9")
        self.mutationProbI.configure(font=font11)
        self.mutationProbI.configure(label="Mutation Probability (%)")
        self.mutationProbI.configure(orient="horizontal")
        self.mutationProbI.configure(troughcolor="#d9d9d9")
        ...
        self.ButtonStart.configure(command=self.startThread)
        ...
        self.ButtonStop.configure(state="disabled")
        self.ButtonStop.configure(command = self.stopThread)
        ...
        self.ButtonOpen.configure(command = self.openOutputFolder)
...
Kyle Wood 16087993''')
        ...
        self.genNumber_11.place(relx=0.86, rely=0.05, height=19, width=180)
